...
const Gallery = () => {
  ...
  const filteredImages = filter === 'all'
    ? autoGalleryImages.slice(0, displayCount)
    : autoGalleryImages.filter(img => img.category === filter).slice(0, displayCount);
  
  const hasMoreImages = filter === 'all'
    ? autoGalleryImages.length > displayCount
    : autoGalleryImages.filter(img => img.category === filter).length > displayCount;
        ...
        <div className="w-full px-0 sm:px-4 md:px-0">
          ...
          {hasMoreImages && (
            <div className="mt-6 sm:mt-8 text-center">
              <button
                onClick={() => setDisplayCount(prev => Math.min(prev + 4, autoGalleryImages.length))}
